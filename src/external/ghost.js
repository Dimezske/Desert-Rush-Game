...
export default class Ghost extends Phaser.Physics.Arcade.Sprite{
    constructor(scene, x, y, scale = 0.5, movementType = "random", time = 20, distance = 20) {
        ...
        this.scene.physics.add.overlap(this.scene.schimitar, this,function(){
            if(this.scene.schimitar.anims.isPlaying==true){
                ...
                this.scene.sound.play('ghost-snare');
    ...
    if(this.health <= 0){
        if(this.scene.ghostFell==false){
            ...
            this.scene.addCarrotAbove(this,0x2549db);
            this.destroy();  
    }
    ...
    monster__ani_setup(){
        this.anims.create({
            key: 'idle',
            frames: this.anims.generateFrameNumbers('ghost-sprite', {start: 2, end: 2 }),
            frameRate: 15,
            repeat: -1
        });
        this.anims.create({
            key: 'left',
            frames: this.anims.generateFrameNumbers('ghost-sprite', {start: 0, end: 1 }),
            frameRate: 10,
            repeat: -1
        });
        this.anims.create({
            key: 'right',
            frames: this.anims.generateFrameNumbers('ghost-sprite', {start: 3, end: 4 }),
            frameRate: 10,
            repeat: -1
        });
    }
    ...
    if(this.health <= 0){
        if(this.scene.ghostFell==false){
            ...
            this.scene.addCarrotAbove(this,0x0037ff);
            this.destroy();  
    }
