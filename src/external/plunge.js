...
export default class Plunge extends Phaser.Physics.Arcade.Sprite{
    constructor(scene, x, y, scale = 0.5, movementType = "random", time = 20, distance = 20) {
        super(scene, x, y, 'plunge-sprite', 0);
        ...
        this.deathTimer;
        /*
        const progressHealth = this.scene.add.graphics();
        
        // Register a load progress event to show a load bar
        this.scene.load.on('progressHealth', (value) => {
            progressHealth.clear();
            progressHealth.fillStyle(0x424983, 1);
            progressHealth.fillRect(this.x, this.y, 50 / this.health, 10);
            console.log(parseInt(progressHealth * 100) + '%');
        });*/
        this.scene.physics.add.overlap(this.scene.player.attackbox, this,function(){
            if(this.scene.player.anims.isPlaying==true){
                this.health-=this.scene.player_punch.damage;
                this.scene.sound.play('plunger-hurt');
                if (this.x < this.scene.player.x ){
                    this.setPosition(this.x-15, this.y)
                    
                    this.dtext = this.scene.add.text(this.x,this.y,`${this.scene.player.damage}`).setTint(0xfc4903);
                ...
                //if(this.scene.player.attackbox.frame.name==1){this.y-=150};
        };
        if(this.health <= 0){
        ...
        this.scene.physics.add.overlap(this.scene.schimitar, this,function(){
            ...
            if (this.x < this.scene.player.x ){
                ...
                this.setPosition(this.x+150, this.y)
                this.dtext = this.scene.add.text(this.x,this.y,`${this.scene.player_punch.damage}`).setTint(0xfc4903);
                this.dtext = this.scene.add.text(this.x,this.y,`${this.scene.schimitar.damage}`).setTint(0xfc4903);
                this.scene.time.addEvent({
                    delay: 1200,
                    callback: () => this.dtext.destroy(),
    ...
    monster__ani_setup(){
        this.anims.create({
            key: 'idle',
            frames: this.anims.generateFrameNumbers('plunge-sprite', {start: 2, end: 2 }),
            frameRate: 15,
            repeat: -1
        });
        this.anims.create({
            key: 'left',
            frames: this.anims.generateFrameNumbers('plunge-sprite', {start: 0, end: 1 }),
            frameRate: 10,
            repeat: -1
        });
        this.anims.create({
            key: 'right',
            frames: this.anims.generateFrameNumbers('plunge-sprite', {start: 3, end: 4 }),
            frameRate: 10,
            repeat: -1
        });
    }
    checkMonsterMovement() {
        ...
        if (this.y > bottomPlatform.y + 200)
        {
            this.health=0;
            this.exp=0;
            this.scene.ghostFell=true;
        }
    if(this.health <= 0){
        if(this.scene.ghostFell==false){
            this.scene.ghostDied=true;
            this.scene.player.exp += this.exp; 
        };
            this.scene.addCarrotAbove(this,0xff8400);
            this.destroy();  
    }
