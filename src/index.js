...
//---------Guns-----------------------------
import P90Img from "./assets/p90.png";
import P90 from "./game/Guns/p90";
import P90_Shoot from "./assets/sfx/p90_fire.wav";

import Bullet from "./game/bullet";
import BulletImg from "./assets/p90_bullet.png";
import WaterCreate from "./environment/water";
/**
...
class GameScene extends Phaser.Scene {
  ...
  init() {
    ...
    this.soundManager = function () {
        ...
        this.growlTimer = this.time.addEvent({
          ...
          callback: function () {
            for (let i = 0; i < this.Plunger.length; i++) {
              let distanceFromPlayer = Phaser.Math.Distance.Between(
                this.Plunger[i].x,
                this.Plunger[i].y,
                this.player.x,
                this.player.y
              ...
              if (
                ...
                this.plungerFarSound.isPlaying == false
              ) {
                this.plungerFarSound.play();
              } else if (
                distanceFromPlayer < 500 &&
                this.plungerSound.isPlaying == false
              ) {
      ...
      if (this.plungerFell == true) {
        this.plungerDeathFarSound.play();
        this.plungerFell = false;
      }
      if (this.plungerDied == true) {
        this.plungerDeathSound.play();
        ...
        this.growlTimer_rat = this.time.addEvent({
          ...
          callback: function () {
            for (let i = 0; i < this.Rats.length; i++) {
              ...
              if (
                distanceFromPlayer > 500 &&
                this.ratFarSound.isPlaying == false
              ) {
                this.ratFarSound.play();
              } else if (
                distanceFromPlayer < 500 &&
                this.ratSound.isPlaying == false
              ) {
                this.ratSound.play();
              }
            }
      ...
      if (this.ratFell == true) {
        this.ratDeathSound.play();
        this.ratFell = false;
      }
      if (this.ratDied == true) {
        this.ratDeathSound.play();
        this.ratDied = false;
      }
        ...
        this.growlTimer_ghost = this.time.addEvent({
          ...
          callback: function () {
            for (let i = 0; i < this.Ghosts.length; i++) {
              ...
              if (
                distanceFromPlayer > 500 &&
                this.ghostFarSound.isPlaying == false
              ) {
                this.ghostFarSound.play();
              } else if (
                distanceFromPlayer < 500 &&
                this.ghostSound.isPlaying == false
              ) {
      ...
      if (this.ghostFell == true) {
        this.ghostDeathFarSound.play();
        this.ghostFell = false;
      }
      if (this.ghostDied == true) {
  ...
  preload() {
    ...
    this.load.json(
      "level_spawn_crystal",
      "src/leveldata/level_spawncrystals.json"
    );
    this.load.json(
      "level_ground_walling",
      "src/leveldata/level_platform_walling.json"
    );
    ...
    this.load.audio("p90-shoot", P90_Shoot);
    this.load.spritesheet("p90-sprite", P90Img, {
      frameWidth: 388,
      frameHeight: 163,
    });
    this.load.image("p90-bullet-sprite", BulletImg);
  }
  ...
  create(data) {
    ...
    this.plungerDeathFarSound = this.sound.add("plunger-death", {
      volume: 0.7,
    });
    ...
    background
      .setScale(Math.max(1450 / background.width, 775 / background.height))
      .setScrollFactor(0);
    // --------------- parallax----------------------------------
    const width = this.scale.width;
    const height = this.scale.height;
    const totalWidth = width * 10;
    this.add.image(width * 0.5, height * 0.5, "forest-sky").setScrollFactor(0);
    ...
    this.spawnCrystalGroup = this.add.group();
    //this.spawnCrystal = [this.physics.add.existing(new SpwnCrystal(this,200,0,'spwn-crystal', 0))]
    this.spawnCrystal = new SpwnCrystal(this, 200, 0, "spwn-crystal", 0);
    this.spawnCrystalGroup.add(this.spawnCrystal);
    ...
    this.player_punch = this.physics.add.existing(
      new Punch(this, -200, 200, "player-punch-sprite", 0)
    );
    ...
    this.schimitar = this.physics.add.existing(
      new Schimitar(this, this.player.x, this.player.y, "schimitar-sprite", 0)
    );
    ...
    this.crate = this.physics.add.existing(
      new Crate(this, 300, -250, "crate", 0)
    );
    //-------------Guns----------------------------------
    this.p90 = this.physics.add.existing(
      new P90(this, this.player.x, this.player.y, "p90-sprite", 0)
    );
    this.p90.cursors = this.input.keyboard.createCursorKeys();
    this.p90.body.allowGravity = false;
    this.p90.body.setBounce(0.2);
    this.p90.setScale(0.14);
    this.p90.setSize(60, 80);
    this.p90.__shoot_ani_setup();

    // /this.bullet = this.physics.add.existing(new Bullet(this, this.p90.muzzlebox.x, this.p90.muzzlebox.y, 'p90-bullet-sprite', 0))
    //-------------------------
    ...
    this.levelData_SpawnCrystals.SpawnCrystalData.forEach((element) => {
      let crystal = new SpwnCrystal(this, element.x, element.y);
      this.spawnCrystalGroup.add(crystal);
    });
    let crystals = this.spawnCrystalGroup.getChildren();
    for (let i = 0; i < crystals.length; i++) {
      crystals[i].crystalID = i;
    }
    this.platform = [];
    this.levelData_level1.platformData.forEach((element) => {
      this.platform.push(
        new Platforms(this, element.x, element.y, "platform", element.scale)
      );
    });
    this.mPlatform = [];
    ...
    this.levelData_Monsters.PlungerData.forEach((element) => {
      let plunger = new Plunge (
          ...
          element.time,
          element.distance);
            this.Plunger.push(plunger)
            this.monsters.add(plunger)
    });
    this.Rats = [];
    ...
    this.physics.add.collider(this.ground_sand, this.Plunger);
    this.physics.add.collider(this.platforms, this.Plunger);
    this.Plunger.forEach((element) =>
      this.physics.add.collider(element, this.plunger)
    );
    ...
    this.Rats.forEach((element) =>
      this.physics.add.collider(element, this.rats)
    );
    ...
    this.Ghosts.forEach((element) =>
      this.physics.add.collider(element, this.ghosts)
    );

    this.platform.forEach((element) =>
      this.physics.add.collider(element, this.player)
    );
    this.mPlatform.forEach((element) =>
      this.physics.add.collider(element, this.player)
    );

    this.player_box_collider = this.physics.add.collider(
      this.player,
      this.crate
    );
    ...
    this.levelData_Clouds.cloudData.forEach((element) =>
      this.Cloud.push(
        new Clouds(this, element.x, element.y, "clouds", element.isAddative)
      )
    );
    emitter.startFollow(this.Cloud[0]);
    this.rCloud = [];
    this.levelData_Clouds = this.cache.json.get("level_clouds");
    this.levelData_Clouds.rain_cloudData.forEach((element) =>
      this.rCloud.push(
        new RainClouds(
          this,
          element.x,
    ...
    this.ballsCollectedText = this.add
      .text(240, 10, "Balls: 0", { color: "white", fontSize: 24 })
      .setScrollFactor(0)
      .setOrigin(0.5, 0);
    ...
    this.physics.add.overlap(
      this.player,
      this.swords,
      this.handleCollectSword,
      undefined,
      this
    );
    this.swordsCollectedText = this.add
      .text(380, 10, "Swords: 0", { color: "yellow", fontSize: 24 })
      .setScrollFactor(0)
      .setOrigin(0.5, 0);
    ...
    //.setScrollFactor(0);
    //document.addEventListener('click', this.p90.shootBullet());
    this.bullets = this.add.group();
    for (let i = 0; i < 30; i++) {
      let bullet = new Bullet(this, 0, 0);
      bullet.setActive(false);
      this.bullets.add(bullet);
      this.p90.bullets.push(bullet);
    }
    this.water = this.physics.add.existing(new WaterCreate(this, 100, 100));
  }
  ...
  update(time, delta) {
    ...
    this.p90.setPosition(this.player.x, this.player.y);
    this.p90.control_handler();

    this.Cloud.forEach((element) => {
      element.checkCloudMovement();
    });
    this.rCloud.forEach((element) => {
      element.checkCloudMovement();
    });
    this.mPlatform.forEach((element) => {
      element.checkPlatformMovement();
    });
    this.HummingBird.forEach((element) => {
      element.checkInsectMovement();
    });
    ...
    for (let i = 0; i < this.Rats.length; i++) {
      if (this.Rats[i].health <= 0) {
      }
    }
    for (let i = 0; i < this.Ghosts.length; i++) {
      if (this.Ghosts[i].health <= 0) {
      }
    }
    this.soundManager();
    ...
    if (this.player.y > bottomPlatform.y + 200) {
      ...
      this.cameras.main.fade(3000, 0xffffff);
      //listen for fade to complete then restart
      this.player.y = -10000;
      ...
      this.input.keyboard.once("keydown-SPACE", () => {
        ...
        this.player.setPosition(
          this.player.respawnPoint.x,
          this.player.respawnPoint.y
        );
        this.cameras.main.fadeIn(3000, 0x8efad8);
      });
    }
  }
